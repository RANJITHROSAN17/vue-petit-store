...
var FictionalDate = class FictionalDate {
  ...
  to_tempos(utc) {
    var D, G, H, M, N, N0, Nn, S, Z, Zy, Zz, after_leap_month, d, e, graph, is_leap_month, is_previous_year, m, now_idx, p, period, s, u, w, y, z0_p, zz_p, zz_q;
    ...
    z0_p = Zz.last_at + this.calc.msec.season * 1; // 正月中気

    N0 = to_tempo_bare(this.calc.msec.moon, this.calc.zero.moon, z0_p); // 正月中気の月

    Nn = period(N0, "moon");
    zz_p = Zz.last_at + this.calc.msec.season * (1 + Nn.now_idx * 2); // 今中気

    zz_q = Zz.last_at + this.calc.msec.season * (3 + Nn.now_idx * 2); // 翌中気

    Zy = to_tempo_bare(this.calc.msec.year, this.dic.start_at, utc);

    if (!(after_leap_month = Nn.next_at + 0 < zz_p)) {
      is_leap_month = Nn.next_at + this.calc.msec.moon < zz_q;
    }
    ...
    D = period(u, "day"); // day    in week (曜日)

    w = to_tempo_bare(this.calc.msec.week, this.calc.zero.week, utc);
    e = period(w, "day"); // hour   in day

    H = period(d, "hour"); // minute in day
    ...
    y = Object.assign({}, u);
    y.now_idx = now_idx;
    graph = `${this.dic.seasons[Z.now_idx]} ${Z.now_idx % 2 ? _.padStart((Z.now_idx + 1) / 2, 2, '0') : "  "}\t ${is_leap_month ? "閏" : "  "}${_.padStart(Nn.now_idx + 1, 2, '0')}月${_.padStart(N.now_idx + 1, 2, '0')}日\t ${y.now_idx} ${Zz.now_idx}`;
    return {
      G,
      u,
      y,
...
EARTH = [[31556925147.0, new Date("2019/03/21 06:58").getTime()], [2551442889.6, new Date("2019/01/06 10:28").getTime()], [to_msec('1d'), 0], 23.4397, [35, 135]];
...
FictionalDate.令和 = g.dup().yeary(['月', '火', '水', '木', '金', '土', '日']).calendar(["令和", "令和前"], "1年5月1日(水)0時0分0秒", new Date("2019-5-1").getTime()).init();
