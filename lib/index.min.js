...
var FictionalDate = class FictionalDate {
  ...
  def_table_by_leap_year() {
    var a, day, div, i, idx, is_leap, j, k, leaps, len, len1, len2, mode, month, month_ranges, month_sum, months, period, q, range, ref, size, str, upto, year, years;
    ...
    for (q = 0, len2 = years.length; q < len2; q++) {
      size = years[q];
  ...
  def_zero() {
    var day, hour, minute, month, moon, o, period, season, season_zero, second, since, week, year, year_size, zero, zero_size;
    ...
    season_zero = zero - this.calc.msec.year * this.calc.idx.season / this.dic.seasons.length;
    ({
      since
  ...
  to_tempos(utc) {
    var D, H, M, S, Z, Zz, d, e, m, n, now_idx, p, period, s, w, y;
    ...
    n = to_tempo_bare(this.calc.msec.moon, this.calc.zero.moon, utc);
    Zz = to_tempo_bare(this.calc.msec.year, this.calc.zero.season, utc); // year   in period
    ...
    D = period(y, "day"); // season in year_of_planet

    Z = period(Zz, "season");
    console.warn([Z.now_idx / 2, M.now_idx]); // day    in week (曜日)
  ...
  to_labels({
    ...
    var G;
    y.label = y.now_idx + p.now_idx * this.calc.divs.period;
    y.label = 0 < y.label ? (G = {
      now_idx: 0
    }, y.label + "年") : (G = {
      now_idx: 1
    }, 1 - y.label + "年");
    G.label = this.dic.era[G.now_idx];
    M.label = this.dic.months[M.now_idx];
    d.label = d.now_idx + 1 + "日";
    H.label = this.dic.hours[H.now_idx];
    m.label = this.dic.minutes[m.now_idx];
    s.label = s.now_idx + "秒";
    S.label = ("" + S.now_idx / this.calc.msec.second).slice(2);
    e.label = this.dic.weeks[e.now_idx];
    Z.label = this.dic.seasons[Z.now_idx];
    return {
      G,
      p,
      y,
  ...
  format(utc, str = default_format_format) {
    var o;
    o = this.to_labels(this.to_tempos(utc));
...
EARTH = [to_msec('1y'), 2551442889.6, to_msec('1d'), new Date("2019/03/21 6:58").getTime(), 23.4397, [35, 135]];
