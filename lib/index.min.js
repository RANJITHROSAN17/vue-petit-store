...
var FictionalDate = class FictionalDate {
  ...
  def_table_by_leap_year() {
    ...
    return function (path, _1, b_size, _3) {
      switch (path) {
        case 'year':
          return year;
  ...
  def_table_by_season() {
    ...
    return function (path, zero, _2, b_idx) {
      return null;
    };
  }
  ...
  def_idx() {
    ...
    console.warn([, year, month, day, week, hour, minute, second] = this.dic.start.match(reg_parse));
    year = year - 0;
  ...
  slice(utc) {
    var D, G, H, M, S, Z, Zy, d, e, last_at, m, n, now, now_idx, p, period, s, size, w, y;
    now = utc - 0 + this.dic.tz_offset;

    period = (base, path) => {
      var b_size, last_at, now_idx, o, size, table;
      table = this.get_table(path, ...base);
      ({
        last_at,
        now_idx,
        size
      } = o = table ? to_tempo_by(table, base[0], now) : (b_size = this.calc.msec[path], to_tempo_bare(b_size, base[0], now)));
    ...
    Z = period(Zy, "season");
    console.warn([Z[2] / 2, M[2]]); // day    in week (曜日)
  ...
  parse(tgt, str = default_parse_format) {
    ...
    return this.calc.zero.period + p * this.calc.msec.period + in_period + d * this.calc.msec.day + H * this.calc.msec.hour + m * this.calc.msec.minute + s * this.calc.msec.second + S - this.dic.tz_offset;
  }
...
EARTH = [to_msec('1y'), 2551442889.6, to_msec('1d'), new Date("2019/03/21 6:58").getTime(), 23.4, [35, 135]];
