...
var FictionalDate = class FictionalDate {
  ...
  def_table() {
    var a, day, div, idx, is_leap, j, k, leaps, len, len1, mode, month, period, ref, size, upto, year, years;
    ...
    this.table.range.year[0] = this.calc.range.year[1];
    years = _.uniq(this.table.range.year);
    this.table.range.month = month = {};
    ...
    upto = function (src) {
      var k, len1, msec, range, results;
      msec = 0;
      results = [];

      for (k = 0, len1 = src.length; k < len1; k++) {
        range = src[k];
        results.push(msec += range * day);
      }

      return results;
    };

    year = upto(this.table.range.year);
    ...
    for (k = 0, len1 = years.length; k < len1; k++) {
      size = years[k];
      month[size * day] = upto(this.table.range.month[size]);
    }

    return this.table.msec = {
      year,
      month
    };
  }
  ...
  def_calc() {
    ...
    minute = this.dic.seconds.length;
  ...
  def_zero() {
    var day, hour, minute, month, moon, period, second, week, year, yeary, zero, zero_size;

    zero_size = (path, idx = 0) => {
      return 0 - (this.calc.idx[path] - idx) * this.calc.msec[path];
    };

    zero = -this.dic.tz_offset;
    ...
    return calc_set.call(this, "zero", {
      period
    });
  }

  slice(now) {
    var D, G, H, M, S, d, e, m, n, p, period, s, w, y;
    ...
    period = ([zero, b_size], path) => {
      ...
      ({
        ...
        switch (path) {
          case 'year':
            table = this.table.msec.year;
            return to_tempo_by(table, zero, now);

          case 'month':
            table = this.table.msec.month[b_size];
            return to_tempo_by(table, zero, now);

          default:
            b_size = this.calc.msec[path];
            return to_tempo_bare(b_size, zero, now);
        }
      }.call(this));
    ...
    p = period([this.calc.zero.period], "period"); // year   in period
    ...
    D = period(y, "day");
    w = period(y, "week");
    n = period(y, "moon"); // day    in week (曜日)
    ...
    s = period(m, "second");
    S = [null, null, now - s[0]];
    G = [null, null, (now < this.calc.zero.period) - 0];
    G[2] = this.dic.era[G[2]];
    p[2] = p[2] * this.calc.divs.period;
    y[2] = y[2] + p[2] + "年";
    M[2] = this.dic.months[M[2]];
    d[2] = d[2] + 1 + "日";
    H[2] = this.dic.hours[H[2]];
    m[2] = this.dic.minutes[m[2]];
    s[2] = this.dic.seconds[s[2]];
...
FictionalDate.Gregorian = g = new FictionalDate().planet(to_msec('1y'), 2551442881, to_msec('1d'), 23.4, [35, 135]).calendar(["西暦", "紀元前"], [4, 100, 400], "1970年1月1日(木)0時0分0秒", 0, 27).yeary(['日', '月', '火', '水', '木', '金', '土'], ['睦月', '如月', '弥生', '卯月', '皐月', '水無月', '文月', '葉月', '長月', '神無月', '霜月', '師走'], [31, 0, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31]).moony(['朔', '既朔', '三日月', '上弦', '上弦', '上弦', '上弦', '上弦', '上弦', '上弦', '上弦', '上弦', '十三夜', '小望月', '満月', '十六夜', '立待月', '居待月', '臥待月', '更待月', '下限', '下限', '下限', '下限', '下限', '下限', '下限', '下限', '晦', '晦']).daily(function () {
  var results = [];
